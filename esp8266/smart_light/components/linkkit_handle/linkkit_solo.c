...
static int user_property_set_event_handler(const int devid, const char *request, const int request_len)
{
    ...
    res = IOT_Linkkit_Report(EXAMPLE_MASTER_DEVID, ITM_MSG_POST_PROPERTY,
                             (unsigned char *)request, request_len);
    ESP_LOGI(TAG,"Post Property Message ID, this is my log: %d", res);

    return SUCCESS_RETURN;
}


int32_t app_post_property_powerstate( bool value)
{
    int32_t res = -0x100;
    ...
    ESP_LOGI(TAG,"send the data to cloud over: %s --------",property_payload);
    return res;
}
                                           ...
                                           int *response_len)
{
    ...
    for (index = 0; index < cJSON_GetArraySize(request_root); index++) {
        ...
        {
            cJSON *item_powerstate = cJSON_CreateObject();
            if (item_powerstate == NULL) {
                cJSON_Delete(request_root);
                cJSON_Delete(response_root);
                return -1;
            }
            cJSON_AddNumberToObject(item_powerstate, "powerstate", lightbulb_get_powerstate());
        }
        else if (strcmp("brightness", item_propertyid->valuestring) == 0)
        {
            cJSON_AddNumberToObject(response_root, "brightness", lightbulb_get_brightness());
        }
    }
    cJSON_Delete(request_root);
        ...
        char **response, int *response_len)
{
    int contrastratio = 0, to_cloud = 0;
    char *time1 = NULL;
    cJSON *root = NULL, *item_transparency = NULL, *item_from_cloud = NULL, *item_time = NULL;
    ESP_LOGI(TAG,"Service Request Received, Devid: %d, Service ID: %.*s, Payload: %s", devid, serviceid_len,
                  serviceid,
                  request);
    ...
    else if (strlen("timerOnOff") == serviceid_len && memcmp("timerOnOff", serviceid, serviceid_len) == 0)
    {
        ESP_LOGI(TAG,"get the timerOnOff");
        time_t timenow = 0;
        time(&timenow);
        ESP_LOGI(TAG,"get the localtime is======= %ld",timenow);

        const char *response_fmt = "{\"time\":%s}";
        item_time = cJSON_GetObjectItem(root, "Time");
        if (item_time == NULL || !cJSON_IsString(item_time)) {
            cJSON_Delete(root);
            ESP_LOGE(TAG,"there is not the time value");
            return -1;
        }
        ESP_LOGI(TAG,"timerOnOff get the time");
        ESP_LOGI(TAG,"Time is : %s", item_time->valuestring);
        time1 = item_time->valuestring;

        /* Send Service Response To Cloud */
        *response_len = strlen(response_fmt) + 15 + 1;
        *response = malloc(*response_len);
        if (*response == NULL) {
            ESP_LOGW(TAG,"Memory Not Enough");
            return -1;
        ...
        memset(*response, 0, *response_len);
        snprintf(*response, *response_len, response_fmt, time1);
        *response_len = strlen(*response);
        ESP_LOGI(TAG,"timerOnOff resp is %s",*response);
    }
...
static int linkkit_thread(void *paras)
{
    ...
    ESP_LOGI(TAG, "go to the yield loop");

    while (1) {
        IOT_Linkkit_Yield(EXAMPLE_YIELD_TIMEOUT_MS);
        //ESP_LOGI(TAG, "this is the yield loop");
    }
    ESP_LOGI(TAG, "out the yield loop");
